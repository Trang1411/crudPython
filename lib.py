...
def read_json_file(json_file, day_run):
    print(f"day run ===== {day_run} và json_file ==== {json_file}")
    if day_run == datetime.date.today().day or day_run == 0:
        ...
        for config_file_json in config_file:
            ...
            print(f"================ body của file {json_file} là {body} ====================")
            ...
            if method == "POST":
                ...
                print(f" file {json_file}, phương thức POST ===== và kết quả là {response_data}")
            if method == "GET":
                response_data = _get(url, headers, body)
                print(f" file {json_file} và phương thức GET ===== kết quả là {response_data}")
            if method == "PUT":
                response_data = _put(url, headers, body)
                print(f" file {json_file} và phương thức PUT ===== kết quả là {response_data}")
            if method == "UPLOAD_FILE":
                with open(file_path, "rb") as image_file:
                    response_data = _upload_file(url, headers, body, {"file": image_file})
                    print(f" file {json_file} phương thức UPLOAD_FILE ===== và kết quả là {response_data}")
            ...
            print()
            print("SUCCESS!!! - ", f"Thời gian chạy {json_file} là  ====================== ", datetime.datetime.now())
            print()
    return
...
def evd(time_run, file_name):
    ...
    print(f"EVD ============== ", file_name)
    job_with_params = partial(read_json_file, file_name, 0)
...
def evt(time_run, file_name):
    print(f"EVT ============== ", file_name)
    job_with_params = partial(read_json_file, file_name, 0)
...
def evm(time_run, file_name):
    ...
    job_with_params = partial(read_json_file, file_name, day_time_run)
...
def deleteInScheduleData(value_del):
    print(f" gia tri cua value_del ======== {value_del}")
    old = readFileScheduleData()  # lấy dữ liệu file scheduleData
    for key in old.keys():
        # lấy giá trị theo key
        values = old[key]  # lấy mảng giá trị của key đó
        # kiểm tra có value_del tồn tại trong scheduleData chưa, nếu có thì thực hiện xóa
        for value in values:
            if value_del == value:
                print(f" value_del {value_del} =========== value {value} ")
                values.remove(value_del)
    # print giá trị mảng sau khi xóa
    print(f"giá trị của scheduleData sau khi xóa {old}")


def addInScheduleData(time_add, service_name):
    old = readFileScheduleData()
    if time_add not in old:
        old[time_add] = [service_name]
    else:
        old[time_add].append(service_name)
